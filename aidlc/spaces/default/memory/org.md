...
We **deploy on merge** to non-production environments. Production deploys
gate on a separate manual approval — typically tech lead + product owner
sign-off.

Deployment path depends on repo type:

| Repo type | CI/CD path | Promotion |
|-----------|------------|-----------|
| Application infrastructure (`tf-aws-*-infrastructure`) | Jenkins on Bitbucket | UrbanCode Deploy (UCD) to AWS per environment |
| TFCOM / TFMOD catalog modules | Jenkins `terraform.IaCTests` | Module release tags — not UCD |
| Application code | `[TBD — Platform/DevOps]` | Team-specific (Jenkins, CodePipeline, or other) |

For Terraform infrastructure repos, see Tier 2 knowledge:
`aidlc-pipeline-deploy-agent/labcorp-jenkins-patterns.md` and
`labcorp-ucd-deploy-flow.md`.
