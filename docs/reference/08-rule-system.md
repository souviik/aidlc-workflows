...
`/aidlc --doctor` ships two advisory rows that observe rule/sensor state. Both are read-only and always pass — neither changes the health-check exit code. (As of v0.6.10 `--doctor` is cold-safe: the `GUARDRAIL_LOADED` audit row the paired-coverage check emits is written only when `aidlc-docs/audit.md` already exists. On a pristine, pre-`--init` project doctor prints the rows but emits nothing and creates no files.)
