...
> **⚠️ Experimental / work in progress.** This document describes the intended
> architecture of the IDE harness. The adapters (Cursor, Cline, Copilot, Windsurf,
> Antigravity) are scaffolding and are **not yet production-ready** — most lack a
> working automation path. For proven execution use the CLI harness (`claude-code`,
> `kiro-cli`) or the Strands execution path.
