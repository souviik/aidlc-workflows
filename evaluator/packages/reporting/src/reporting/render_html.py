...
def render_html(data: ReportData) -> str:
    ...
    w(
        f'<div class="subtitle"><code>{_esc(run_name)}</code> &middot; {_esc(data.generated_at)}</div>'
    )

    # ── Test metadata ──────────────────────────────────────────
    w('<table style="margin-bottom:1.5rem">')
    w(
        f"<tr><td><strong>Executor Model</strong></td><td><code>{_esc(data.meta.executor_model)}</code></td></tr>"
    )
    w(
        f"<tr><td><strong>Simulator Model</strong></td><td><code>{_esc(data.meta.simulator_model)}</code></td></tr>"
    )
    if data.meta.rules_source == "git" and data.meta.rules_repo:
        w(
            f"<tr><td><strong>Rules Source</strong></td><td><code>{_esc(data.meta.rules_repo)}</code> @ <code>{_esc(data.meta.rules_ref)}</code></td></tr>"
        )
    elif data.meta.rules_source == "local" and data.meta.rules_local_path:
        w(
            f"<tr><td><strong>Rules Source</strong></td><td>local: <code>{_esc(data.meta.rules_local_path)}</code></td></tr>"
        )
    elif data.meta.rules_source:
        w(
            f"<tr><td><strong>Rules Source</strong></td><td><code>{_esc(data.meta.rules_source)}</code></td></tr>"
        )
    w("</table>")
    ...
    if data.tests:
        ...
        w(
            f'<div class="card-value">{_badge(f"{t.pass_pct:.1f}% ({t.passed}/{t.total})", cls)}{_delta_tag(cmp, "Tests Pass %")}</div>'
        )
        if t.coverage_pct is not None:
            w(
                f'<div class="card-detail">Coverage: {t.coverage_pct:.1f}%{_delta_tag(cmp, "Coverage %")}</div>'
            )
        w("</div>")
    ...
    if data.contracts:
        ...
        w(
            f'<div class="card-value">{_badge(f"{ct.passed}/{ct.total}", cls)}{_delta_tag(cmp, "Contract Passed")}</div>'
        )
    ...
    if data.quality:
        ...
        w(
            f'<div class="card-value">{_badge(f"{q.lint_total} lint / {q.security_total} security", cls)}{_delta_tag(cmp, "Lint Errors")}</div>'
        )
    ...
    w(
        f'<div class="card-value">{_ms_to_human(data.metrics.wall_clock_ms)}{_delta_tag(cmp, "Wall Clock (ms)")}</div>'
    )
    ...
    w(
        f'<div class="card-value">{_fmt_tokens(data.metrics.total_tokens.total_tokens)}{_delta_tag(cmp, "Total Tokens")}</div>'
    )
    w(
        f'<div class="card-detail">in: {_fmt_tokens(data.metrics.total_tokens.input_tokens)} / out: {_fmt_tokens(data.metrics.total_tokens.output_tokens)}</div>'
    )
    w("</div>")
    ...
    rows = [
        ...
        (
            "Handoffs",
            f"{data.meta.total_handoffs} ({' &rarr; '.join(_esc(n) for n in data.meta.node_history)})",
        ),
    ]
    ...
    if data.metrics.handoffs:
        ...
        w(
            '<div style="display:flex;gap:2px;height:32px;border-radius:8px;overflow:hidden;margin-bottom:1rem">'
        )
        ...
        for h in data.metrics.handoffs:
            ...
            w(
                f'<div style="width:{pct:.1f}%;background:{col};display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:600;min-width:30px" title="{_esc(h.node_id)} #{h.handoff}: {_ms_to_human(h.duration_ms)}">{_esc(h.node_id[0].upper())}{h.handoff}</div>'
            )
        w("</div>")
        ...
        for h in data.metrics.handoffs:
            ...
            w(
                f'<tr><td class="num">{h.handoff}</td><td>{_esc(h.node_id)}</td>'
    ...
    w(
        "<table><tr><th>Agent</th><th class='num'>Input</th><th class='num'>Output</th><th class='num'>Total</th></tr>"
    )
    for name, tok in [
        ...
        w(
            f"<tr><td>{name}</td><td class='num'>{_fmt_tokens(tok.input_tokens)}</td>"
    ...
    if data.metrics.repeated_context_tokens.total_tokens > 0:
        ...
        w(
            f"<tr><td>Repeated Context</td><td class='num'>{_fmt_tokens(data.metrics.repeated_context_tokens.input_tokens)}</td>"
        ...
        w(
            f"<tr><td><strong>API Total</strong></td><td class='num'><strong>{_fmt_tokens(data.metrics.api_total_tokens.input_tokens)}</strong></td>"
    ...
    if ctx_total and ctx_total.sample_count > 0:
        ...
        if ctx_ex and ctx_ex.sample_count > 0:
            w(
                f"<tr><td>Executor</td><td class='num'>{_fmt_tokens(ctx_ex.min_tokens)}</td>"
        ...
        if ctx_si and ctx_si.sample_count > 0:
            w(
                f"<tr><td>Simulator</td><td class='num'>{_fmt_tokens(ctx_si.min_tokens)}</td>"
        ...
        w(
            f"<tr><td><strong>Total</strong></td><td class='num'><strong>{_fmt_tokens(ctx_total.min_tokens)}</strong></td>"
    ...
    if data.tests:
        ...
        w(
            f"{_badge(f'{t.pass_pct:.1f}% passed ({t.passed}/{t.total})', 'pass' if test_ok else 'fail')}"
        )
        if t.coverage_pct is not None:
        ...
        w("</div>")
        w(
            f'<div class="progress-wrap"><div class="progress-bar {_progress_class(ratio)}" style="width:{ratio * 100:.1f}%"></div></div>'
        )
    ...
    if data.contracts:
        ...
        for group_name, cases in groups.items():
            ...
            w(
                f"<h3>{_esc(group_name.title())} {_badge(f'{passed_g}/{total_g}', 'pass' if ok_g else 'fail')}</h3>"
            )
            w(
            ...
            for c in cases:
                ...
                w(
                    f'<tr><td class="{icon_cls}"></td><td>{_esc(c.name)}</td><td>{c.method}</td>'
    ...
    if data.quality:
        ...
        w(
            f"{_badge(f'{q.security_total} security findings', 'pass' if q.security_high == 0 else 'fail')}"
        )
        if q.lint_available:
        ...
        if q.lint_findings:
            ...
            w(
                "<table><tr><th>File</th><th class='num'>Line</th><th>Code</th><th>Message</th><th>Severity</th></tr>"
            )
            for f in q.lint_findings:
                sev_cls = "fail" if f.severity == "error" else "warn"
                w(
                    f'<tr><td><code>{_esc(f.file)}</code></td><td class="num">{f.line}</td>'
    ...
    if data.qualitative:
        ...
        if ql.phases:
            ...
            for phase in ql.phases:
                w(
                    f'<div class="phase-bar-group"><div class="phase-bar-title">{_esc(phase.phase)}</div>'
                )
                for dim, val in [
                    ...
                    w(
                        f'<div class="bar-row"><div class="bar-row-label">{dim}</div>'
        ...
        for phase in ql.phases:
            ...
            for d in phase.documents:
                if d.notes:
                    name = Path(d.path).name
                    w(
                        f"<details><summary><code>{_esc(name)}</code> &mdash; {d.overall:.2f}</summary>"
                    )
    ...
    if data.comparison:
        ...
        w(
            f'<div style="color:var(--text2);font-size:.85rem;margin-bottom:1rem">'
        ...
        w(
            f'<div class="cmp-stat"><div class="cmp-stat-val delta-improved">{cmp.improved}</div><div class="cmp-stat-label">Improved</div></div>'
        )
        w(
            f'<div class="cmp-stat"><div class="cmp-stat-val delta-regressed">{cmp.regressed}</div><div class="cmp-stat-label">Regressed</div></div>'
        )
        w(
            f'<div class="cmp-stat"><div class="cmp-stat-val delta-unchanged">{cmp.unchanged}</div><div class="cmp-stat-label">Unchanged</div></div>'
        )
        w("</div>")
        ...
        for d in cmp.deltas:
            ...
            w(
                f'<tr><td><span class="delta-dot {dot_cls}"></span></td>'
                f"<td>{_esc(d.name)}</td>"
