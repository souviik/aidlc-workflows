...
def render_markdown(data: ReportData) -> str:
    ...
    if data.tests:
        w(
            f"| Unit Tests | {_status_icon(test_ok)} **{data.tests.pass_pct:.1f}%** ({data.tests.passed}/{data.tests.total}){_md_delta(cmp, 'Tests Pass %')} |"
        )
    if data.contracts:
        w(
            f"| Contract Tests | {_status_icon(contract_ok)} **{data.contracts.passed}/{data.contracts.total}** passed{_md_delta(cmp, 'Contract Passed')} |"
        )
    if data.quality:
        q_ok = data.quality.lint_errors == 0 and data.quality.security_high == 0
        w(
            f"| Code Quality | {_status_icon(q_ok)} lint: {data.quality.lint_total} ({data.quality.lint_errors} errors), security: {data.quality.security_total} ({data.quality.security_high} high){_md_delta(cmp, 'Lint Errors')} |"
        )
    if data.qualitative:
        w(
            f"| Qualitative Score | {'🟢' if qual_score >= 0.8 else '🟡' if qual_score >= 0.6 else '🔴'} **{qual_score:.2f}**{_md_delta(cmp, 'Qualitative Score')} |"
        )
    w(
        f"| Execution Time | {_ms_to_human(data.metrics.wall_clock_ms)}{_md_delta(cmp, 'Wall Clock (ms)')} |"
    )
    w(
        f"| Total Tokens | {_fmt_tokens(data.metrics.total_tokens.total_tokens)}{_md_delta(cmp, 'Total Tokens')} |"
    )
    w("")
    ...
    w("|-------|------:|-------:|------:|")
    w(
        f"| Executor | {_fmt_tokens(data.metrics.executor_tokens.input_tokens)} | {_fmt_tokens(data.metrics.executor_tokens.output_tokens)} | {_fmt_tokens(data.metrics.executor_tokens.total_tokens)} |"
    )
    w(
        f"| Simulator | {_fmt_tokens(data.metrics.simulator_tokens.input_tokens)} | {_fmt_tokens(data.metrics.simulator_tokens.output_tokens)} | {_fmt_tokens(data.metrics.simulator_tokens.total_tokens)} |"
    )
    w(
        f"| **Total Unique** | **{_fmt_tokens(data.metrics.total_tokens.input_tokens)}** | **{_fmt_tokens(data.metrics.total_tokens.output_tokens)}** | **{_fmt_tokens(data.metrics.total_tokens.total_tokens)}** |"
    )
    w("")
    ...
    if data.metrics.repeated_context_tokens.total_tokens > 0:
        ...
        w("|----------|------:|-------:|------:|")
        w(
            f"| Repeated Context | {_fmt_tokens(data.metrics.repeated_context_tokens.input_tokens)} | {_fmt_tokens(data.metrics.repeated_context_tokens.output_tokens)} | {_fmt_tokens(data.metrics.repeated_context_tokens.total_tokens)} |"
        )
        w(
            f"| **API Total** | **{_fmt_tokens(data.metrics.api_total_tokens.input_tokens)}** | **{_fmt_tokens(data.metrics.api_total_tokens.output_tokens)}** | **{_fmt_tokens(data.metrics.api_total_tokens.total_tokens)}** |"
        )
        w("")
    w("")
    ...
    if ctx_total and ctx_total.sample_count > 0:
        ...
        if ctx_ex and ctx_ex.sample_count > 0:
            w(
                f"| Executor | {_fmt_tokens(ctx_ex.min_tokens)} | {_fmt_tokens(ctx_ex.max_tokens)} | {_fmt_tokens(ctx_ex.avg_tokens)} | {_fmt_tokens(ctx_ex.median_tokens)} | {ctx_ex.sample_count} |"
            )
        if ctx_si and ctx_si.sample_count > 0:
            w(
                f"| Simulator | {_fmt_tokens(ctx_si.min_tokens)} | {_fmt_tokens(ctx_si.max_tokens)} | {_fmt_tokens(ctx_si.avg_tokens)} | {_fmt_tokens(ctx_si.median_tokens)} | {ctx_si.sample_count} |"
            )
        w(
            f"| **Total** | **{_fmt_tokens(ctx_total.min_tokens)}** | **{_fmt_tokens(ctx_total.max_tokens)}** | **{_fmt_tokens(ctx_total.avg_tokens)}** | **{_fmt_tokens(ctx_total.median_tokens)}** | **{ctx_total.sample_count}** |"
        )
        w("")
    ...
    if data.quality:
        ...
        w("")
        w(
            f"**{_status_icon(q_ok)} Lint: {q.lint_total} findings** ({q.lint_errors} errors, {q.lint_warnings} warnings)"
        )
        w("")
        ...
        if q.duplication_available:
            ...
            w(
                f"**{_status_icon(dup_ok)} {q.duplication_blocks} duplicate block(s)** ({q.duplication_lines} duplicated lines)"
            )
        else:
    ...
    if data.qualitative:
        ...
        for phase in ql.phases:
            ...
            for d in phase.documents:
                ...
                w(
                    f"| `{name}` | {d.intent:.2f} | {d.design:.2f} | {d.completeness:.2f} | {d.overall:.2f} |"
                )
            w("")
