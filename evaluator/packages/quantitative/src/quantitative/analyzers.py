...
import defusedxml.ElementTree as ET  # noqa: N817 — ET is the universal ElementTree alias
...
def run_ruff(project_root: Path) -> ToolResult:
    ...
    if version is None:
        return ToolResult(
            tool="ruff", version=None, available=False, error="ruff not found on PATH or via uv"
        )
    ...
    try:
        ...
        return ToolResult(tool="ruff", version=version, available=True, error="timed out")
    ...
    try:
        ...
        for item in items:
            ...
            try:
                # nosemgrep: ai.ai-best-practices.hooks-path-traversal - relative_to() enforces path stays within project_root; ValueError on escape  # noqa: E501
...
def run_bandit(project_root: Path) -> ToolResult:
    ...
    if version is None:
        return ToolResult(
            tool="bandit", version=None, available=False, error="bandit not found on PATH or via uv"
        )
    ...
    try:
        ...
        return ToolResult(tool="bandit", version=version, available=True, error="timed out")
...
def run_eslint(project_root: Path) -> ToolResult:
    ...
    if version is None:
        ...
        if npx is None:
            return ToolResult(
                tool="eslint", version=None, available=False, error="eslint/npx not found on PATH"
            )
    ...
    try:
        ...
        return ToolResult(tool="eslint", version=version, available=True, error="timed out")
...
def run_npm_audit(project_root: Path) -> ToolResult:
    ...
    if npm is None:
        return ToolResult(
            tool="npm-audit", version=None, available=False, error="npm not found on PATH"
        )
    ...
    if not lock_file.exists():
        return ToolResult(
            tool="npm-audit", version=version, available=True, error="no package-lock.json found"
        )
    ...
    try:
        ...
        return ToolResult(tool="npm-audit", version=version, available=True, error="timed out")
...
def run_semgrep(project_root: Path) -> ToolResult:
    ...
    # Scan src/ if it exists to avoid scanning .venv/, tests/, and other noise.
    # Fall back to project_root only when there is no src/ layout.
    src_dir = project_root / "src"
    scan_target = str(src_dir) if src_dir.is_dir() else str(project_root)

    try:
        result = _run_tool(
            prefix + ["scan", "--config", "auto", "--json", scan_target],
            cwd=project_root,
        )
    except subprocess.TimeoutExpired:
        return ToolResult(tool="semgrep", version=version, available=True, error="timed out")
...
def run_cpd(
    ...
    for dirpath, dirnames, _ in os.walk(abs_root):
        matched = [d for d in dirnames if d in cpd_excludes]
        for d in matched:
        ...
        dirnames[:] = [d for d in dirnames if d not in cpd_excludes]
    ...
    try:
        ...
        return ToolResult(tool="pmd-cpd", version=None, available=True, error="timed out")
    ...
    try:
        if result.stdout.strip():
            ...
            for dup in root.findall(f"{ns}duplication"):
                ...
                codefragment = (
                    (codefragment_elem.text or "").strip() if codefragment_elem is not None else ""
                )
                findings.append(
                    DuplicationFinding(
                        files=files,
