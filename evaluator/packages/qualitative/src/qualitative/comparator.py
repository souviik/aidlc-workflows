...
def compare_runs(
    ...
    *,
    ...
    # Pass the scorer's Bedrock client to pair_documents so it can use LLM
    # matching for candidate docs that don't match the golden by path.
    bedrock_client = getattr(scorer, "_client", None)
    llm_model_id = getattr(scorer, "_model_id", model_id)
    ...
    known_phase_order = ("inception", "construction", "other")
    ordered_phases = [p for p in known_phase_order if p in phase_documents]
    ordered_phases += sorted(p for p in phase_documents if p not in known_phase_order)
