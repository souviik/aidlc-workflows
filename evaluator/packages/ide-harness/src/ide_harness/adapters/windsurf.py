...
class WindsurfAdapter(IDEAdapter):
    ...
    def run(self, config: AdapterConfig) -> AdapterResult:
        ...
        try:
            ...
            logger.info(
                "Monitoring workspace for aidlc-docs/ output (timeout=%ds, poll=%ds, stable=%ds)",
            ...
            if completed:
                logger.info("Windsurf run completed successfully in %.1fs", elapsed)
                return AdapterResult(
                    success=True,
                    output_dir=config.output_dir,
...
def _build_agents_md(config: AdapterConfig) -> str:
    ...
    if config.rules_path.is_dir():
        ...
        rules_text = "\n\n---\n\n".join(f.read_text(encoding="utf-8") for f in rule_files)
    else:
...
def _check_sentinels(workspace_dir: Path) -> bool:
    ...
    inception_ok = any((workspace_dir / s).exists() for s in _INCEPTION_SENTINELS)
    # At least one construction sentinel must exist.
    construction_ok = any((workspace_dir / s).exists() for s in _CONSTRUCTION_SENTINELS)
    # Both tracking files should exist.
    tracking_ok = all((workspace_dir / s).exists() for s in _TRACKING_SENTINELS)
