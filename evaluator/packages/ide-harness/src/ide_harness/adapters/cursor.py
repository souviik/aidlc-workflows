...
class CursorAdapter(IDEAdapter):
    ...
    def check_prerequisites(self) -> tuple[bool, str]:
        ...
        return (
            False,
            f"Cursor CLI ('{_AGENT_CLI}') not found in PATH. Install the Cursor agent CLI first.",
        )
    ...
    def run(self, config: AdapterConfig) -> AdapterResult:
        ...
        try:
            ...
            workspace_dir = Path(tempfile.mkdtemp(prefix="aidlc-cursor-"))
            ...
            if proc.returncode != 0:
                error_detail = (
                    f"agent CLI exited with code {proc.returncode}. stderr: {raw_stderr[:2000]}"
                )
            ...
            logger.info("Cursor run completed successfully in %.1fs", elapsed)
            return AdapterResult(
                success=True,
                output_dir=config.output_dir,
                aidlc_docs_dir=_aidlc_docs_if_exists(config.output_dir),
            ...
            elapsed = time.monotonic() - start_time
            error_msg = f"Cursor agent CLI timed out after {config.timeout_seconds}s"
