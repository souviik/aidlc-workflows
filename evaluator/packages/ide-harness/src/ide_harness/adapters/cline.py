...
class ClineAdapter(IDEAdapter):
    ...
    def check_prerequisites(self) -> tuple[bool, str]:
        ...
        try:
            ...
            cline_installed = any(ext.strip().lower() == _CLINE_EXTENSION_ID for ext in extensions)
    ...
    def run(self, config: AdapterConfig) -> AdapterResult:
        ...
        try:
            ...
            logger.info("INSTRUCTIONS.md written (%d bytes)", len(instructions_content))
            ...
            if completed and aidlc_docs:
                logger.info("Cline run completed successfully in %.1fs", elapsed)
                return AdapterResult(
                    success=True,
                    output_dir=config.output_dir,
    ...
    def _poll_for_output(
        ...
        while True:
            ...
            if elapsed >= timeout_seconds:
                logger.warning("Polling timed out after %.1fs", elapsed)
                ...
                return (
                    aidlc_docs.is_dir()
                    and _count_files(aidlc_docs) >= _MIN_AIDLC_FILES_FOR_COMPLETION
                )
