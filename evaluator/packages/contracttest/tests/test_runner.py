...
import httpx
import yaml
from contracttest.runner import ContractTestResults, _match_body, _run_case, write_results
...
class TestRunCase:
    ...
    def test_wrong_status(self):
        case = TestCase(name="not found", method="GET", path="/missing", expected_status=404)
        mock_resp = MagicMock()
        mock_resp.status_code = 200
    ...
    def test_connection_error(self):
        case = TestCase(name="health", method="GET", path="/health", expected_status=200)
        client = MagicMock()
