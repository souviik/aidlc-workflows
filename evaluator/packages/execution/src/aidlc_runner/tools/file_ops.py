...
def make_readonly_file_tools(run_folder: Path) -> list:
    """Create read-only file operation tools bound to a specific run folder.

    Used for agents (e.g. orchestrator) that should never write artifacts.

    Returns:
        List of tool-decorated functions: [read_file, list_files].
    """
    run_folder = run_folder.resolve()
