#!/usr/bin/env python3
"""Extension Hook Test Runner — test AIDLC evaluations with extension opt-in configs.
...
def run_evaluation_with_config(
    ...
    print(
        f"  [{status.upper()}] {config_name} — {elapsed_min:.1f} min (exit code {result.returncode})"  # noqa: E501
    )
    ...
    if run_folder:
        ...
        print("  [WARN] No run folder found after execution")
...
def main() -> None:
    ...
    parser.add_argument(
        "--rules-ref",
        default="feat/extension_hook_question_split",
        help="Git ref for AIDLC rules with extension hook support (default: feat/extension_hook_question_split)",  # noqa: E501
    )
    ...
    if args.configs:
        ...
        if missing:
            parser.error(
                f"Unknown configs: {', '.join(missing)}. Use --list-configs to see available options."  # noqa: E501
            )
    else:
