...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Base config YAML providing AWS, swarm, and scorer defaults (default: config/default.yaml)",  # noqa: E501
    )
    parser.add_argument(
        "--profile",
        default=None,
    ...
    if args.models == "all":
        ...
        for name in selected:
            if name not in available:
                parser.error(f"Unknown model '{name}'. Available: {', '.join(available.keys())}")

    batch_start = time.monotonic()
    batch_started_at = datetime.now(UTC).isoformat(timespec="seconds")
    results: list[dict] = []

    print("AIDLC Batch Evaluation")
